class_name MultiGridContainer extends Control
...
#根据id新建物品并放置到多格子容器中
func add_new_item_at(cell_pos: Vector2, item_id: int) -> bool:
...
#根据id给WItem设置基础数据
func set_item_data_at_id(item: WItem, item_id: int) -> void:
	var data: Dictionary = AutoLoad.find_item_data(item_id)
	if data:
		item.set_data(data)
	else:
