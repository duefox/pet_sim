class_name WGrid extends TextureRect
...
func _ready() -> void:
	MouseEvent.mouse_cell_matrix = self.parent_cell_matrix
...
func _on_mouse_entered() -> void:
	#向鼠标事件实例对象传递信息
	MouseEvent.mouse_cell_pos = self.cell_pos
	MouseEvent.mouse_cell_matrix = self.parent_cell_matrix
	pass  # Replace with function body.
...
func _on_gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		...
		if event.button_index == MOUSE_BUTTON_LEFT && event.is_pressed() && !MouseEvent.is_mouse_drag():
			GlobalData.previous_cell_matrix = self.parent_cell_matrix
			var cur_item_data: ItemData = self.parent_cell_matrix.get_grid_map_item(self.cell_pos)
			...
			if cur_item_data.is_placed:
				...
				self.parent_cell_matrix.set_item_placed(cur_item, false)
				#设置放置提示框，并显示
