...
## random_pos随机坐标
## wander_rank容器可漫游的范围
func create_pet(pet_container: PetRoom, pet_scene: PackedScene, pet_data: Dictionary, random_pos: Vector2, wander_rank: Rect2) -> Node:
	var new_pet: Pet = pet_scene.instantiate()
	pet_id_counter += 1
	new_pet.position = random_pos
	pets[new_pet.pet_id] = new_pet
	# 向宠物容器中添加宠物
	pet_container.add_pet(new_pet)
	new_pet.initialize_pet(pet_id_counter, pet_data, wander_rank)
	# 调用成长函数切换动画
	new_pet.grow_up()
