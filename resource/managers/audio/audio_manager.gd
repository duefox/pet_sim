...
func preload_audio(path: String, type: AudioType) -> void:
	if not _audio_cache.has(path):
		...
		if audio_resource:
			_audio_cache[path] = {"resource": audio_resource, "type": type}
		resource_loaded.emit(path, audio_resource)
...
## 暂停音频
## [param path] 音频路径
func pause(path: String) -> void:
	var audio_resource = _get_audio_resource(path)
	for type in _audio_players:
		for player: AudioStreamPlayer in _audio_players[type]:
			if player.stream == audio_resource && player.playing:
				player.stream_paused = true


## 停止播放音频
## [param path] 音频路径
func stop(path: String) -> void:
	var audio_resource = _get_audio_resource(path)
	for type in _audio_players:
		for player: AudioStreamPlayer in _audio_players[type]:
			if player.stream == audio_resource && player.playing:
...
func _get_audio_resource(path: String) -> AudioStream:
	...
	var audio_resource = load(path)
	if audio_resource:
		_audio_cache[path] = {"resource": audio_resource, "type": AudioType.SOUND_EFFECT}
...
func _fade_in_music(duration: float) -> void:
	if _current_music:
		...
		var tween = audio_node_root.create_tween()
		tween.tween_property(_current_music, "volume_db", linear_to_db(_volumes[AudioType.MUSIC]), duration)
