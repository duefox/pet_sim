...
func _ready() -> void:
	...
	if not root:
		return
	# 连接资源加载完成信号
	if not ResManager.resource_loaded.is_connected(_on_resource_loaded):
...
func preload_scene(scene_path: String) -> void:
	...
	ResManager.load_resource(scene_path, ResManager.LOAD_MODE.LAZY)
...
func change_scene_async(
	...
	# 防止同时切换多个场景
	if _is_switching:
		push_warning("Scene switch already in progress, ignoring request to switch to: %s" % scene_path)
		return
	...
	if stack_index >= 0:
		...
		# 加载新场景
		new_scene = ResManager.get_instance(scene_path)
		if not new_scene:
			var scene_resource : PackedScene = ResManager.get_cached_resource(scene_path)
...
func add_sub_scene(
	...
	var scene_resource = ResManager.load_resource(scene_path)
...
func clear_preloaded_scenes() -> void:
	for scene_path in _preloaded_scenes:
		ResManager.clear_resource_cache(scene_path)
...
func register_transition(effect: TransitionEffect, transition: BaseTransition, custom_name: StringName = "") -> void:
	...
	if effect == TransitionEffect.CUSTOM:
		if custom_name.is_empty():
			push_warning("Custom transition name cannot be empty")
...
func _start_transition(effect: TransitionEffect, duration: float, custom_transition_name: StringName = "") -> void:
	...
	if effect == TransitionEffect.CUSTOM and custom_transition_name:
		...
		if effect in _transitions:
			...
			push_warning("Transition effect not found: %d" % effect)
...
## 清理转场效果
func _cleanup_transition(_effect: TransitionEffect, _custom_transition_name: StringName = "") -> void:
	_transition_rect.visible = false
...
func _do_scene_switch(
	...
	if not scene_data.is_empty():
		if new_scene.has_method("init_state"):
			...
			push_warning("Scene %s does not have an init_state method" % new_scene.get_name())
